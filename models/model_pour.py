...
class ConcatModel(nn.Module):
    def __init__(self):
        ...
        self.audio_net.load_state_dict(loaded_dict, strict=False)
        self.visual_net.load_state_dict(loaded_dict, strict=False)
        self.tact_net.load_state_dict(loaded_dict, strict=False)
        del loaded_dict
        
        self.audio_net.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                                   bias=False)
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(128 * 6 * 3 + 64, 1024),
            torch.nn.ReLU(),
        ...
        self.target_mlp = nn.Embedding(num_embeddings=200, embedding_dim=64)

    def forward(self, audio, visual, tactile, action=None, weight_prompt=None):
        ...
        wt = self.target_mlp(weight_prompt).squeeze(1)
        
        out = torch.cat([a,v,t], dim=1)
        
        out = torch.flatten(out, 1)
        out = torch.cat([out, wt], dim=1)
        out = self.mlp(out)
...
class MULSA(nn.Module):
    ...
    def forward(self, audio, visual, tactile, actions=None, weight_prompt=None):
...
class MSBot(nn.Module):
    def __init__(self, blur_p = 0.25, beta = 0.5):
        ...
        self.state_tokenizer = nn.Linear(128*4+64, 128)
        ...
        self.lstm = torch.nn.LSTM(4, 128, 1, batch_first=True)
        ...
        self.gate = torch.nn.Linear(128,4)
        
    def forward_state(self, audio, visual, tactile, actions, weight_prompt):
        EPS = 1e-8
        B = visual.size()[0]
        ...
        state_token = torch.cat([a[:,-1],v[:,-1],t[:,-1],h.squeeze(1),wt], dim=1).flatten(1)
        state_token = self.state_tokenizer(state_token).unsqueeze(1)
        
        return state_token

    def forward(self, audio, visual, tactile, actions, weight_prompt, warmup=False):
        #print(self.all_stage_tokens)
        EPS = 1e-8
        B = visual.size()[0]
