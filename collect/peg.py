...
# [155.888107, -154.827805, 204.574417, -88.201238, 2.251266, 178.022742] init 
# [122.005592, -154.905548, 409.702332, 177.898296, 88.019495, 86.024227] prepared
# z = 408.7

def on_press(key):
    # global global_time
    # if (time.time() - global_time) >= 0.5:
...
def func_listen():
    with open('data_peg/00record.txt', 'w') as f:
        global press, press_type, init, f_img, f_tactile
        while True:
            if init:
...
def func_img():
    ...
    with open('data_peg/01img.txt', 'w') as f:
        f_img = f
        while True:
            frame = pipeline.wait_for_frames()
            ...
            cv2.imwrite('data_peg/rgb/'+str(i)+'_rgb.png', color_image)
            cv2.imwrite('data_peg/depth/'+str(i)+'_dep.png', depth_colormap)
...
def func_tactile():
    ...
    with open('data_peg/02tactile.txt', 'w') as f:
        f_tactile = f
        while dev.while_condition:
            f1 = dev.get_image(roi)
            cv2.imwrite('data_peg/tactile/'+str(i)+'.png', f1)
...
if __name__ == '__main__':
    if not os.path.exists('data_peg'):
        os.mkdir('data_peg')
    if not os.path.exists('data_peg/rgb'):
        os.mkdir('data_peg/rgb')  
    if not os.path.exists('data_peg/depth'):
        os.mkdir('data_peg/depth') 
    if not os.path.exists('data_peg/tactile'):
        os.mkdir('data_peg/tactile')   
    ip = "192.168.1.224"
